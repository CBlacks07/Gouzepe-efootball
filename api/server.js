...
/* ====== DB schema (auto + migrations tolérantes, SANS FK durs) ====== */
async function ensureSchema(){
  ...
  // Ajoute id si absent (IDENTITY), puis remplit les lignes existantes
  await q(`ALTER TABLE users ADD COLUMN IF NOT EXISTS id INTEGER GENERATED BY DEFAULT AS IDENTITY`);
  await q(`UPDATE users SET id = DEFAULT WHERE id IS NULL`);
  // Rend email unique si pas déjà
  await q(`DO $$ BEGIN
    IF NOT EXISTS (
  ...
  // -- PLAYERS (pas de FK strict vers users)
  await q(`CREATE TABLE IF NOT EXISTS players(
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT now()
  )`);
  await q(`ALTER TABLE players ADD COLUMN IF NOT EXISTS user_id INTEGER`);
...
app.post('/auth/login', async (req,res)=>{
  try{
    ...
    // IMPORTANT : on suppose maintenant que users.id existe (créé plus haut)
