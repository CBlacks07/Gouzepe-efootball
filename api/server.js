...
async function ensureSchema(){
  // players d'abord (pour FK ensuite)
...
/* ---------- seasons helpers + calculs standings ---------- */
...
app.get('/players/:pid/summary', auth, async (req,res)=>{
  ...
  const days = await q(`SELECT day,payload FROM matchday WHERE season_id=$1 OR season_id IS NULL ORDER BY day ASC`,[seasonId]);
  ...
  try{
    const st=await computeSeasonStandings(seasonId);
    const ix=st.findIndex(x=>x.id===req.params.pid);
    if(ix>=0){ rank=ix+1; points=st[ix].total; moyenne=st[ix].moyenne; }
  }catch(_){}

  ok(res,{ season_id:seasonId, player:{ player_id:req.params.pid, name:r.rows[0].name }, legs, goals_for:gf, goals_against:ga, best, rank, points, moyenne });
});
app.get('/players/:pid/matches', auth, async (req,res)=>{
  ...
  ok(res,{ season_id:seasonId, matches: out });
});
...
async function loadDaysCurOrLegacy(){
  const s = await currentSeasonId();
  const r = await q(`SELECT day,payload FROM matchday
                     WHERE season_id=$1 OR season_id IS NULL
                     ORDER BY day ASC`, [s]);
  return { season_id: s, rows: r.rows };
}
...
app.get('/me/player', auth, async (req,res)=>{
  ...
  const p = await q(`SELECT player_id,name,role,profile_pic_url FROM players WHERE player_id=$1`,[pid]);
  ok(res,{ player:p.rows[0]||null });
});
app.get('/me/matches', auth, async (req,res)=>{
  const pid = await getLinkedPlayerId(req.user.uid);
  if(!pid) return bad(res,404,'Aucun joueur lié');
  const { season_id, rows } = await loadDaysCurOrLegacy();
  const out=[];
...
app.get('/me/stats', auth, async (req,res)=>{
  ...
  let rank=null, points=null, moyenne=null;
  try{
    const st=await computeSeasonStandings(await currentSeasonId());
  ...
  ok(res,{ season_id, played_legs:played, goals_for:GF, goals_against:GA, best_single:best, season_rank:rank, season_points:points, season_average:moyenne });
});
...
/* ---------- standings ---------- */
app.get('/standings', auth, async (_req,res)=>{
  const sid = await currentSeasonId();
  const list = await computeSeasonStandings(sid);
  ok(res,{ season_id:sid, standings:list });
});
app.get('/season/standings', auth, async (_req,res)=>{
  const sid = await currentSeasonId();
  const list = await computeSeasonStandings(sid);
  ok(res,{ season_id:sid, standings:list });
});
...
/* ---------- presence (socket réservé) ---------- */
io.on('connection', (socket)=>{ /* placeholders */ });
