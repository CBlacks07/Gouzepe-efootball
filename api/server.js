...
if (process.env.DATABASE_URL) {
  pool = new Pool({
    ...
    ssl: (process.env.PGSSL === 'true') || process.env.RENDER === 'true' || process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false,
  });
} else {
  const useSSL = (process.env.PGSSL === 'true') || process.env.RENDER === 'true' || process.env.NODE_ENV === 'production';
  pool = new Pool({
...
async function ensureSchema(){
  ...
  // --- migrations draft.author_user_id (compat older DB) ---
...
app.get('/healthz', (req,res)=>{
  res.json({ ok:true, db: 'ready', ts: Date.now() });
});
/* ====== Start ====== */
(async ()=>{
  try{
